...
class ModelConfig:
    ...
    trading_model: str = "ssvi_surface"  # Model for trading probabilities: "ssvi_surface" or "heston"
...
class MonteCarloConfig:
    """Monte Carlo simulation configuration for terminal probability calculations."""
...
class Config:
    ...
    def from_yaml(cls, path: Path) -> "Config":
        ...
        if "heston" in data:
            ...
            # Strip removed config fields that may still be in old YAML files
            _removed_heston = {
                "calibration_integration_points", "ultra_short_dated_ttm_threshold",
                "ultra_short_dated_xi_bounds", "ultra_short_dated_kappa_bounds",
                "ultra_short_dated_theta_factor", "short_ttm_gaussian_weighting",
                "short_ttm_gaussian_sigma_base", "short_ttm_gaussian_sigma_ttm_scale",
                "short_ttm_gaussian_floor", "quantlib_objective_impl",
                "enable_numba_fallback", "numba_strict_mode", "use_relative_error",
            }
            for k in _removed_heston:
                heston_data.pop(k, None)
            config.heston = HestonConfig(**heston_data)

        if "model" in data:
            model_data = data["model"].copy()
            model_data.pop("fallback_to_ssvi", None)
        ...
        # Support both "monte_carlo" (new) and "barrier" (old) config keys
        mc_data = data.get("monte_carlo", data.get("barrier"))
        if mc_data:
            config.monte_carlo = MonteCarloConfig(**mc_data)
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "heston": {
                ...
                "max_workers": self.heston.max_workers,
            },
            "model": {
                "default_model": self.model.default_model,
                "trading_model": self.model.trading_model,
            ...
            "monte_carlo": {
                ...
                "n_steps_per_day": self.monte_carlo.n_steps_per_day,
                "confidence_level": self.monte_carlo.confidence_level,
                "use_antithetic": self.monte_carlo.use_antithetic,
            },
            "ssvi_surface": {
