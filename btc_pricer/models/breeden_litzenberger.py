...
class BreedenLitzenberger:
    ...
    def _extract_core(
        ...
        # Calculate forward call prices at each strike
        call_prices = np.zeros(len(strikes))
        for i, K in enumerate(strikes):
            vol = surface.implied_volatility_strike(K, forward)
            call_prices[i] = BlackScholes.forward_call_price(forward, K, vol, ttm)
