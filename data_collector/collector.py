...
from scripts.market_utils import (
    fetch_current_market,
    get_time_remaining_hours,
)
...
def log_probabilities_to_csv(
    ...
    row = {
        ...
        "reference_price": poly_data.get("barrier"),
...
def run_edge_check(probabilities_path: Path, verbose: bool = False,
                   market: dict = None) -> None:
    """Run a full edge check: model calibration + Polymarket data + CSV log.

    Pipeline order matches trader.py: Model first (slow ~50s), then Polymarket
    (fast, gets freshest prices right before edge comparison).
    """
    ...
    terminal_json = COLLECTOR_DIR / "results" / "terminal_data.json"
    poly_json.parent.mkdir(parents=True, exist_ok=True)

    try:
        # Get barrier/TTM from market dict if available, otherwise fetch Polymarket
        if market and market.get("reference_price"):
            barrier = market["reference_price"]
            hours_remaining = get_time_remaining_hours(market)
        else:
            log("Step 0/3: Fetching Polymarket for barrier/TTM...")
            pre_poly = run_polymarket_script(verbose=verbose, json_path=poly_json)
            if pre_poly["barrier"] is None:
                ...
                return
            barrier = pre_poly["barrier"]
            hours_remaining = pre_poly["hours_remaining"]

        log(f"Step 1/3: Running model calibration for ${barrier:,.0f}, "
            f"{hours_remaining:.2f}h TTM...")
        model_data = run_terminal_script(
            barrier, hours_remaining,
            verbose=verbose, json_path=terminal_json,
            market_end_time=market.get("end_date") if market else None,
        )

        if model_data.get("error"):
            log(f"[ERROR] Terminal script error: {model_data['error'][:200]}")
        ...
        log(f"Step 1/3: OK - Model P(above)={model_data['prob_above']:.4f}, "
            f"P(below)={model_data['prob_below']:.4f}")

        # Fetch Polymarket prices AFTER model calibration for freshest comparison
        log("Step 2/3: Fetching Polymarket prices...")
        ...
        if poly_data["barrier"] is None or poly_data["prob_up"] is None:
            elapsed = int((datetime.now(timezone.utc) - check_start).total_seconds())
            log(f"Edge check failed ({elapsed}s): Could not get Polymarket data")
            return

        log(f"Step 2/3: OK - UP={poly_data['prob_up']:.4f}, "
            f"DOWN={poly_data['prob_down']:.4f}")

        log("Step 3/3: Calculating edges and logging to CSV...")
...
def run(once: bool = False, verbose: bool = False) -> None:
    ...
    while True:
        ...
        if should_edge_check:
            run_edge_check(probabilities_path, verbose=verbose, market=current_market)
