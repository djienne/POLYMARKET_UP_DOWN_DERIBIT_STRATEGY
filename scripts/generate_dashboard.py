...
def build_html(config: dict, opt_config: dict, state: dict, prob_rows: list[dict]) -> str:
    ...
    reference_data = json.dumps([float(r.get("reference_price", r.get("barrier_price", 0))) for r in recent_probs])
    ...
    for t in closed_positions:
        ...
        <tr>
            ...
            <td>${t.get('reference_price', t.get('barrier_price', 0)):,.2f}</td>
    ...
    for p in open_positions:
        ...
        <tr>
            ...
            <td>${p.get('reference_price', p.get('barrier_price', 0)):,.2f}</td>
...
<div class="grid">
  ...
  <div class="card">
    <div class="card-title">Reference Price</div>
    <div class="card-value">${current_market.get('reference_price', current_market.get('barrier_price', 0)):,.2f}</div>
...
new Chart(document.getElementById('spotChart'), {{
  ...
  data: {{
    ...
    datasets: [
      ...
      {{ label: 'Reference Price', data: {reference_data}, borderColor: '#d29922', borderDash: [6,3], borderWidth: 2, pointRadius: 0, tension: 0 }},
    ]
  }},
  options: {{
    responsive: true,
...
def main():
    ...
    parser.add_argument("--config", type=Path, default=Path("config_optimize.json"))
    parser.add_argument("--opt-config", type=Path, default=Path("config_optimize.json"))
    parser.add_argument("--state", type=Path, default=None)
    parser.add_argument("--probabilities", type=Path, default=Path("data_collector/results/probabilities.csv"))
