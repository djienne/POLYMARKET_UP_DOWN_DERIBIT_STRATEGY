...
def _build_prob_segments(sorted_trades, df):
    """Build chronologically-ordered probability segments for traded markets.

    Returns list of (ts_list, model_up, market_up, model_down, market_down, trade_list) tuples.
    Each segment contains both UP and DOWN probabilities for the barrier.
    """
    markets = group_markets(df)
    ...
    segments = []
    for barrier in sorted(barrier_trades.keys(),
                          key=lambda b: barrier_trades[b][0].entry_timestamp):
        trade_list = barrier_trades[barrier]
        rows = markets.get(barrier, [])
        if not rows:
            continue
