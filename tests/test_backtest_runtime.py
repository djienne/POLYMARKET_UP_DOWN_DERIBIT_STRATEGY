...
def test_alpha_floor_triggers_entry_when_model_has_edge():
    """Continuous edge function should trigger entry when model_prob >= required."""
    # model_prob_up=60 (0.60), poly_prob_up=40 (0.40)
    # With alpha=1.5, floor=0.35:
    #   required = max(0.35, 1 - (1 - 0.40)^1.5) = max(0.35, 1 - 0.4648) = max(0.35, 0.5352) = 0.5352
    #   model_prob=0.60 >= 0.5352 => has_edge=True
    rows = [
        _row("2026-01-01T00:00:00Z", model_prob_up=60.0, poly_prob_up=40.0),
        _row("2026-01-01T01:00:00Z", time_remaining_hours=0.0, spot_price=105_000.0),
    ]
...
def test_alpha_floor_suppresses_entry_when_model_prob_too_low():
    """Continuous edge function should NOT trigger entry when model_prob < required."""
    # model_prob_up=45 (0.45), poly_prob_up=40 (0.40)
    # With alpha=1.5, floor=0.35:
    #   required = max(0.35, 1 - (1 - 0.40)^1.5) = 0.5352
    #   model_prob=0.45 < 0.5352 => has_edge=False
    rows = [
        _row("2026-01-01T00:00:00Z", model_prob_up=45.0, poly_prob_up=40.0),
        _row("2026-01-01T01:00:00Z", time_remaining_hours=0.0, spot_price=105_000.0),
    ]
...
def test_alpha_floor_floor_suppresses_low_market_prob():
    """Floor parameter should suppress entry when market_prob < floor."""
    # model_prob_up=90 (0.90), poly_prob_up=20 (0.20)
    # With floor=0.35: market_prob=0.20 < floor=0.35 => has_edge=False
    rows = [
        _row("2026-01-01T00:00:00Z", model_prob_up=90.0, poly_prob_up=20.0),
        _row("2026-01-01T01:00:00Z", time_remaining_hours=0.0, spot_price=105_000.0),
    ]
...
def test_legacy_edge_still_works_without_alpha():
    """Legacy edge ratio mode should still work when alpha is not set."""
    rows = [
        _row("2026-01-01T00:00:00Z", edge_up=3.0, poly_prob_up=40.0),
        _row("2026-01-01T01:00:00Z", time_remaining_hours=0.0, spot_price=105_000.0),
    ]
    ...
    trades, _ = run_backtest(
        ...
        trail_activation=0.20,
        trail_distance=0.10,
        # No alpha/floor params => legacy mode
    )
