...
class TestRequiredModelProb:
    """Tests for required_model_prob() with default alpha=1.5, floor=0.35."""

    def test_market_50(self):
        """At market_prob=0.50, raw ≈ 0.646 > floor 0.35 → raw applies."""
        assert required_model_prob(0.50) == pytest.approx(0.646, abs=0.002)
    ...
    def test_market_10_floor(self):
        """At market_prob=0.10, raw ≈ 0.146 < floor 0.35 → floor applies."""
        assert required_model_prob(0.10) == 0.35

    def test_market_0_returns_floor(self):
        """Edge case: market_prob=0 → floor."""
        assert required_model_prob(0.0) == 0.35
...
class TestHasEdge:
    """Tests for has_edge() with default alpha=1.5, floor=0.35."""
    ...
    def test_below_floor_false(self):
        """model=0.34 vs market=0.10 → floor 0.35 → False."""
        assert has_edge(0.34, 0.10) is False

    def test_above_floor_but_market_below_floor_false(self):
        """model=0.50 vs market=0.30 → market < floor 0.35 → False."""
        assert has_edge(0.50, 0.30) is False
    ...
    def test_model_below_market_rejected(self):
        """Regression: model=72.4% < market=76.5% with low alpha/floor allowed entry.
        With alpha<1 the curve sits below identity, so required_model_prob < market_prob.
        The guard must reject when model thinks outcome is less likely than market."""
        assert has_edge(0.724, 0.765, alpha=0.8, floor=0.70) is False

    def test_model_equal_market_rejected(self):
        """model == market → no edge, should be rejected."""
        assert has_edge(0.80, 0.80, alpha=1.5, floor=0.35) is False
